...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      `}</style>
      {/* Rotating sweep line — Optimized with gradient glow instead of shadow */}
      <div
        style={{ transformOrigin: "right center" }}
        className="animate-radar-spin absolute right-1/2 top-1/2 z-40 flex h-[10px] w-[400px] items-center justify-center overflow-hidden bg-transparent"
      >
        <div className="relative z-40 h-[1.5px] w-full" style={{ background: 'linear-gradient(to right, transparent, var(--accent), transparent)' }} />
        {/* Soft bleed layer for glow */}
        <div className="absolute inset-x-0 h-[4px] blur-sm" style={{ background: 'rgba(102, 224, 245, 0.2)' }} />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(102, 224, 245, ${0.3 - (idx + 1) * 0.03})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
